...
def main(args):
    ...
    for subdir in tqdm( os.listdir(args.input_dir) ):
        ...
        done_exp_file = os.path.join(args.input_dir, subdir, "0_fold", f"step{args.step}", "cross_valid_record.csv")
        ...
        for top_metric in tqdm(top_metrics):
            ...
            os.system(f"python {python_fpath} --input_csv {predicted_df_path} --proxy_name {top_metric} --output_csv {predicted_df_path}")
        ...
        # ordered 
        # sorted_cols = sorted(sorted_cols, key=lambda x: int(x[0].split('_')[-1]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dir", type=str, default="/ai4sciense/users/yupei/Biomni_dev/forward_learning/zero-shot/mem_trans_gptoss-claude/")
    parser.add_argument("--data_dir", type=str, default="/ai4sciense/users/yupei/data/ProteinGym/ProteinGym_split_63")
