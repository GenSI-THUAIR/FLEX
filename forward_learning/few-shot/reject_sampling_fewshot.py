...
def main(args):
    ...
    for i in range(args.n_fold):
        ...
        for python_file in tqdm(os.listdir(method_dir)):
            ...
            json_fapth = os.path.join(gather_dir, f"{proxy_name}.json")
            json_dict = None
            with open(json_fapth, "r") as f:
                json_dict = json.load(f)
            selected_columns = json_dict["selected_columns"]
    
            # trained_model_path = os.path.join(model_dir, f"{proxy_name}.joblib")
            python_id = int(proxy_name.split("_")[-1] )

            # train model with fold data
            trained_model_path = os.path.join(fold_valid_dir, f"{subdir}_step{step}_fold{i}_model{python_id}.joblib")
            os.system(f"python {python_fpath} --input_csv {train_df_path} --ckpt_path {trained_model_path}")

            added_df = add_proxy_metric_column(added_df, test_df, trained_model_path, proxy_name, selected_columns)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dir", type=str, default="/AIRvePFS/ai4science/users/yupei/Biomni_dev/test_cases/2025.10.9-10.12/self_evolve_corr_20251017155836/A4GRB6_PSEAI_Chen_2020/0_fold/step1")
